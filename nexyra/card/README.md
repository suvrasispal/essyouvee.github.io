...
The card starts creating a short link as soon as it loads, and the share sheet shows it (e.g. `https://tinyurl.com/26pv4yhn`) with its own Copy button. WhatsApp, Email, Copy link, LinkedIn, the native share sheet and the QR code all use the short link; if a target is tapped before it is ready, the card waits for it rather than sending the long one. TinyURL is tried first, then is.gd, then cleanuri; if all are unreachable it falls back to the full link and says so.

Short links are only created for a hosted card — a `file://` path can't be shortened.
